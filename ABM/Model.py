...
type_of_game= 2 #1 is random game board each game, 2 is costum game board
height = 5
width = 5
removed_list = []
...
def number_of_moves(self):
    """for calculating the number of possible moves for current board state"""
...
def get_list_of_possible_moves(self):
    """
    :param self:
    :return: returns list of all posible moves to make
    """
    moves=[]
    agent_positions = [a.pos for a in self.schedule.agents]
...
class nim_squared(Model):
    ...
    def __init__(self, height, width, type_of_game):
        ...
        self.schedule = SimultaneousActivation(self) #pegs removed at the same time
        self.player = 1
        self.winner = 0
    ...
    def step(self):
        self.schedule.step()
        make_move(self) #makes move
        get_list_of_possible_moves(self)
        self.player = 1 + ((self.player) % 2) #change player each turn
